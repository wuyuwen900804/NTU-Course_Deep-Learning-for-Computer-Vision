...
class DDIM:
    ...
    # use DDIM to sample
    @torch.no_grad()
    def sample(self, batch_size=10, ddim_timesteps=50, ddim_eta=0.0, clip_denoised=True, noises="hw2_data/face/noise"):
        ...
        tensors = [torch.load(os.path.join(f"{noises}/", filename)) for filename in filenames]
...
def output_img(img_num=10, eta=0):
    ...
    with torch.no_grad():
        x_gen = ddim.sample(
            batch_size=img_num, ddim_timesteps=50, ddim_eta=eta, clip_denoised=True, noises=noise_dir
        )
