...
def output_img(img_num=10, eta=0):
    ...
    with torch.no_grad():
        x_gen = ddim.sample(batch_size=img_num, ddim_timesteps=50, ddim_eta=eta, clip_denoised=True)
...
def calculate_mse():
    img_dir, GT_dir = "part2/Output_folder/", "hw2_data/face/GT/"
    ...
    for i, (generated_path, ground_truth_path) in enumerate(zip(img, GT)):
        ...
        # convert to 0 ~ 255
        img, GT = (img * 255).int(), (GT * 255).int(),
        assert img.shape == GT.shape

        # calculate MSE
        mse = torch.nn.functional.mse_loss(img.float(), GT.float())
        print(f"Image pair {i} MSE: {mse.item():5f}")

    save_image(compare_img_list, img_dir + f"compare.png")


def output_eta_compare():
    img_dir = "part2/Output_folder/"
